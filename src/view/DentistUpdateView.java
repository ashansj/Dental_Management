...
public class DentistUpdateView extends javax.swing.JFrame {
    ...
    private void loadTableData() {
        String doctorFullName = DocName.getText().replace("Dr. ", "").trim(); 
        List<Schedule> schedules = controller.loadDentistSchedules(doctorFullName);
        
        DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
        model.setRowCount(0); 
        
        for (Schedule s : schedules) {
            model.addRow(new Object[]{s.getDate(), s.getStartTime(), s.getEndTime(), s.getStatus()});
        }
    }
    ...
    private void UpdateDentestStatusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_UpdateDentestStatusActionPerformed
        ...
        String docName = DocName.getText().replace("Dr. ", "").trim();
        ...
        // දත්ත Controller එකට යැවීම
        String result = controller.updateDentistStatus(docName, workDate, status, startTime, endTime);
        
        if (result.equals("SUCCESS")) {
            JOptionPane.showMessageDialog(this, "Data inserted successfully");
            loadTableData(); // Table එක Refresh කිරීම
        } else if (result.equals("ERR_DATE")) {
            JOptionPane.showMessageDialog(this, "Please select a date.");
        } else {
